...
class EnvironmentValidator {
    ...
    static sanitizeString(value, maxLength = 500) {
        if (!value)
            return undefined;
    ...
    static validateApiKey(value, keyName) {
        if (!value)
            return undefined;
        ...
        if (suspiciousPatterns.some(pattern => pattern.test(value))) {
            SecurityLogger.logSecurityEvent(SecurityEventType.SUSPICIOUS_VALUE, { keyName, reason: 'Contains suspicious patterns' }, true);
            return undefined;
        }
        ...
        if (value.length < 10 || value.length > 200) {
            SecurityLogger.logSecurityEvent(SecurityEventType.INVALID_FORMAT, { keyName, reason: 'Invalid length' }, true);
            return undefined;
        }
        return EnvironmentValidator.sanitizeString(value);
    }
    // Validate URL format
    static validateUrl(value, allowedHosts = []) {
        if (!value)
            return undefined;
        try {
            ...
            if (process.env.NODE_ENV === 'production' && url.protocol !== 'https:') {
                SecurityLogger.logSecurityEvent(SecurityEventType.INVALID_FORMAT, { reason: 'Non-HTTPS URL in production' });
                return undefined;
            }
            // Check allowed hosts if specified
            if (allowedHosts.length > 0 && !allowedHosts.includes(url.hostname)) {
                SecurityLogger.logSecurityEvent(SecurityEventType.INVALID_FORMAT, { reason: 'Host not in allowlist', hostname: url.hostname });
                return undefined;
            }
            return EnvironmentValidator.sanitizeString(value);
        }
        catch {
            SecurityLogger.logSecurityEvent(SecurityEventType.INVALID_FORMAT, { reason: 'Invalid URL format' });
            return undefined;
        }
    }
    ...
    static validateEmail(value) {
        ...
        if (!emailRegex.test(value)) {
            SecurityLogger.logSecurityEvent(SecurityEventType.INVALID_FORMAT, { reason: 'Invalid email format' });
            return undefined;
        }
        return EnvironmentValidator.sanitizeString(value);
    }
    ...
    static validateBoolean(value, defaultValue = false) {
        ...
        if (['false', '0', 'no', 'off'].includes(normalized))
            return false;
        SecurityLogger.logSecurityEvent(SecurityEventType.INVALID_FORMAT, { reason: 'Invalid boolean value', value: normalized });
        return defaultValue;
    }
}
...
class SecureEnvironmentLoader {
    ...
    static loadAndValidate() {
        ...
        REQUIRED_VARIABLES.forEach(varName => {
            if (!process.env[varName]) {
                ...
                SecurityLogger.logSecurityEvent(SecurityEventType.MISSING_REQUIRED_VAR, { variable: varName });
            }
        });
        ...
        const config = {
            ...
            features: {
                publicProfiles: {
                    baseUrl: EnvironmentValidator.validateUrl(process.env.PUBLIC_PROFILES_BASE_URL, ['getmycv-ai.web.app', 'localhost']) || 'https://getmycv-ai.web.app/cv'
                },
    ...
    static performHealthCheck() {
        ...
        if (config.elevenLabs.apiKey)
            healthyServices++;
        if (config.videoGeneration.didApiKey || config.videoGeneration.synthesiaApiKey || config.videoGeneration.heygenApiKey || config.videoGeneration.runwaymlApiKey)
            healthyServices++;
        if (config.search.serperApiKey)
            healthyServices++;
        if (config.rag.openaiApiKey && config.rag.pineconeApiKey)
            healthyServices++;
...
exports.environmentUtils = {
    ...
    isServiceAvailable: (serviceName) => {
        ...
        if (typeof service === 'object' && service !== null) {
            return Object.values(service).some(value => value !== undefined && value !== null && value !== '');
        }
        return false;
    }
};
