...
// ARCHITECTURAL VIOLATION: Core cannot import from recommendations
// This service should be moved to a higher layer module
// import { embeddingService } from '@cvplus/recommendations/embedding.service';
const embeddingService = {
  generateEmbedding: async (_text: string): Promise<number[]> => {
    // Stub implementation - this service should be moved to higher layer
    return [];
  },
...
- Respect privacy and don't make assumptions beyond what's explicitly stated`;
  ...
  private _calculateConfidence(chunks: any[]): number {
    ...
    // Convert to 0-1 scale (assuming scores are typically 0.7-0.95 for good matches)
    return Math.min(Math.max((avgScore - 0.7) / 0.25, 0), 1);
  }
